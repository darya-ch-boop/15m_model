...
def prepare_dataset(symbols, sequence_length=10):
    all_data = []
    if len(symbols) == 0:
        raise IndexError("symbol's length must not be 0")
    for symbol in symbols:
        df = get_binance_data(symbol, interval="15m", limit=2000)
...
if __name__ == "__main__":
    # Define the model
    model = BiRNNModel(input_size=1, hidden_layer_size=115, output_size=1, num_layers=4, dropout=0.3)
